...
def get_all_parents(species):
    ...
    while current_taxid != 1:
        ...
        if taxid2parent.has_key(current_taxid):
...
def taxid2names(taxid):
	if taxid2name.has_key(taxid):
		return taxid2name[taxid]	
def name_taxid(name):
	if name2taxid.has_key(name):
		return name2taxid[name]

		
#print(get_children_multiple(taxid2names(2), 1))
#print(get_parent_multiple(taxid2names(2), 1))
